    ...
    const fetchBoardData = async () => {
      ...
      try {
        ...
        const response = await apiClient.get<PlayBoardResponse>(
          `/v3/racla/board/wjmax/${keyMode}/${board}/player/${userData.playerId}`,
          {
        ...
        createLog('error', 'Error in fetchBoardData', { ...userData })
      } finally {
        if (isMounted) {
          setIsLoading(false)
        }
              ...
              <div className='tw:flex tw:flex-col tw:gap-4 tw:rounded-lg tw:p-6 tw:w-1/3 tw:border tw:border-slate-200 tw:dark:border-slate-700'>
                ...
                <div className='tw:flex tw:gap-2'>
                  {['4B', '4B_PLUS', '6B', '6B_PLUS'].map((mode) => (
                    <Link
                      key={`mode_${mode}`}
                      to={`/games/wjmax/board/${mode}/${board}`}
                      className={`tw:flex tw:items-center tw:justify-center tw:relative tw:px-2 tw:py-1 tw:border tw:border-opacity-50 tw:transition-all tw:duration-500 tw:rounded-md tw:flex-1 ${
                        mode === keyMode
                  ...
                  {levelGroups.map((group) => (
                    ...
                    >
                      {group.levels.map((level) => (
                        <Link
                          key={`level_${level}`}
                          to={`/games/wjmax/board/${keyMode}/${level}`}
                        ...
                        >
                          <div className='tw:flex tw:gap-2'>
                            <ScorePopupComponent
                              ...
                              judgementType={String(keyMode).includes('_PLUS') ? 1 : 0}
                              isVisibleCode={true}
                            />
