        ...
        try {
          ...
          if (!isVarchive) {
            const response = await axios.get(
              `${process.env.NEXT_PUBLIC_API_URL}/v2/racla/songs/djmax_respect_v/${title}/player/${userData.userNo}`,
              {
                headers: {
        ...
        >
          <div className='tw-flex tw-gap-2'>
            ...
            <div className='tw-flex tw-flex-col'>
              ...
              <div className='tw-flex tw-flex-col tw-gap-2 tw-w-80 tw-p-2 tw-rounded-md tw-mb-1 tw-bg-gray-600 tw-bg-opacity-25'>
                {['NM', 'HD', 'MX', 'SC'].map(
            ...
            {rivalName && rivalName !== vArchiveUserData.userName && rivalSongData && (
              <div className='tw-flex tw-flex-col'>
                ...
                {rivalName !== '' && (
                  <span className='tw-text-xs tw-font-light tw-text-gray-300 tw-my-2'>
                    <span className=''>{rivalName}</span>님의 V-ARCHIVE 성과 기록
                  </span>
                )}
              </div>
            )}
