...
export default function VArchiveLoginPage() {
  ...
  const handleRaFileSelect = () => {
    showNotification(
      'RACLA 데스크톱 앱으로 생성한 player.txt 파일을 선택해주세요.',
      'tw-bg-blue-600',
    )
  ...
  const handleVArchiveFileSelect = () => {
    showNotification(
      'V-ARCHIVE 공식 클라이언트로 생성한 account.txt 파일을 선택해주세요.',
      'tw-bg-blue-600',
    )
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        const ipcResult = window.ipc.send('create-player-file', {
          userNo: response.data.userNo,
          userToken: response.data.userToken,
        })
        showNotification('계정 생성이 완료되었습니다.', 'tw-bg-lime-600')
        setNickname('')
        const ipcResult2 = window.ipc.login({
          userNo: response.data.userNo,
          userToken: response.data.userToken,
          vArchiveUserNo: null,
          vArchiveUserToken: null,
                      ...
                      <div className='tw-flex tw-justify-center tw-gap-3 tw-items-start tw-text-sm tw-text-gray-300 tw-mt-6 tw-pt-8 tw-border-t tw-border-gray-700'>
                        ...
                        <div className='tw-flex tw-flex-col tw-gap-1 tw-break-keep'>
                          ...
                          <span
                            ...
                            onClick={() =>
                              window.ipc.send('openBrowser', 'https://v-archive.net/downloads')
                            }
                          >
                      ...
                      <div className='tw-flex tw-justify-center tw-gap-3 tw-items-start tw-text-sm tw-text-gray-300 tw-mt-6 tw-pt-8 tw-border-t tw-border-gray-700'>
                        ...
                        <div className='tw-flex tw-flex-col tw-gap-1 tw-break-keep'>
                          <span>
                            ...
                            중인 운영체제의 사용자 계정 문서 폴더에 위치한 RACLA에 저장되며 추후
                            로그인 시 사용됩니다.
                          </span>
                          <span>
                            계정 생성 시 서비스를 이용을 위한 플레이 데이터, 닉네임 정보, 접속
                            환경의 IP를 RACLA가 수집하는 것을 동의한 것으로 간주합니다. 해당 정보는
                            제3자에게 사용자의 동의 없이 제공되지 않습니다.
                          </span>
                        </div>
                      </div>
                    </div>
