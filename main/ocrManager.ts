...
class OCRApiService {
  ...
  async uploadForOCR(formData: FormData, where: string): Promise<any> {
    ...
    const baseUrl = this.isProd ? 'https://api.racla.app/api' : 'https://api.racla.app/api'
  ...
  try {
    ...
    if (gameCode === 'djmax_respect_v') {
      ...
      const backupResponse = await customAxios.get(
        `${isProd ? 'https://api.racla.app/proxy' : 'https://api.racla.app/proxy'}?url=https://v-archive.net/api/archive/${
