...
const CLIENT_ID =
  process.env.NODE_ENV === 'production' ? '1331547515744682036' : '1333772448528728085'
  ...
  private async getRecentScore(): Promise<RecentScore | null> {
    try {
      const response = await customAxios.get(
        `${process.env.NODE_ENV === 'production' ? 'https://near.r-archive.zip/api' : 'https://noah.r-archive.zip/api'}/v2/play/history/${this.userData.userNo}/djmax_respect_v`,
        {
          headers: {
